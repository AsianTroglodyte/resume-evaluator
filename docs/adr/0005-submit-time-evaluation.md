...
- Execution is **asynchronous** (queued worker) for **workspace practice** and **assignment submit**; the evaluation row’s status reflects `pending` → `processing` → `completed` | `failed` until evaluation finishes.
...
- Instructors may view submissions at **any status**; evaluation feedback appears when the linked evaluation is `completed`, with status shown while pending or failed.
