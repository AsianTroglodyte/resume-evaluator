...
Students own personal **workspaces** for resume iteration and automated scanning, independent of any module or assignment. A workspace is where drafting happens: upload or revise resume versions, run scans, review LLM feedback, and keep a short history of versions and scan results.

**Submission** is a separate LMS action: the student promotes a **workspace snapshot** into an assignment. The snapshot freezes the chosen resume version, scan result, job context, and assignment rule fields at submit time (see freeze-history / rule-snapshot ADRs). Iteration in the workspace does not mutate an existing submission unless the student explicitly resubmits (see single active submission ADR).
...
For MVP, instructors are not required to provide manual feedback; automated scan output attached to the submission snapshot is the primary evaluation artifact for progress tracking.
